...
> **两个版本可选**
> - **`main` 分支（当前）**：完整功能，含内核 HID filter 驱动，需开启 Windows 测试签名。适合开发者自己用。
> - **`driverless-keymap` 分支**：无需任何驱动，直接拷 exe 即可用。适合交付给别人。但缺返回/音量±三个键，且映射的键会误吞物理键盘同名键。

> 📌 **要在其他电脑上部署？** `driverless-keymap` 分支已备好可直接交付的压缩包说明；`main` 分支请看 [`DEPLOY.md`](DEPLOY.md)。
