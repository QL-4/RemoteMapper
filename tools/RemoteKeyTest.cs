// RemoteKeyTest.cs - compare known Up key, then verify Volume+=>F13, Volume-=>F14, Back=>F15
using System;
...
class RemoteKeyTest {
    ...
    static readonly uint[] expected={0x26,0x7C,0x7D,0x7E};
    static readonly string[] physical={"方向上（对照）", "音量加", "音量减", "返回键"};
    ...
    static IntPtr Hook(int nCode, IntPtr wParam, IntPtr lParam) {
        if(nCode>=0 && armed) {
            ...
            if(down && !got) {
                ...
                // Swallow only F13/F14/F15. A wrong physical-keyboard key remains usable.
                if(k.vk>=0x7C && k.vk<=0x7E)return (IntPtr)1;
            }
    ...
    static void Main() {
        ...
        int passed=0;
        Print("=== 小米遥控器四键驱动测试 ===");
        Print("先测方向上作对照，再测驱动映射的三个键；每键等待 15 秒。\n");
        ...
        Print(passed==expected.Length ? "对照键与三个驱动映射键全部成功。" : "测试未全部通过，请保留窗口并告知结果。");
